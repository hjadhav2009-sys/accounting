...
def services() -> tuple[TemplateStudioService, DocumentRepository]:
    ...
    connect = psycopg_connection_factory(url)
...
def family_detail(family_id: UUID, context: Annotated[RequestContext, Depends(request_context)]):
    service, _ = services(); result = service.repository.get_family(context.organization_id, context.company_id, family_id)
...
def samples(version_id: UUID, context: Annotated[RequestContext, Depends(request_context)]):
    service, _ = services(); return {"items": service.repository.samples(context.organization_id, context.company_id, version_id)}
...
def execute_background_test(context: RequestContext, version_id: UUID, run_id: UUID) -> None:
    service, _ = services(); service.execute_test_run(studio_context(context), version_id, run_id)
...
def test_run(run_id: UUID, context: Annotated[RequestContext, Depends(request_context)]):
    service, _ = services(); result = service.repository.test_run(context.organization_id, context.company_id, run_id)
...
def draft_from_document(payload: DraftRequest, context: Annotated[RequestContext, Depends(request_context)]):
    service, documents = services(); document = documents.get(context.organization_id, context.company_id, payload.document_id)
